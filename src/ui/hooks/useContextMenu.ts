...
export function useContextMenuManager({
  ...
  function saveToLibraryContextTarget() {
    if (!contextMenu || contextMenu.kind !== 'component') return;
    saveComponentToLibrary(contextMenu.componentId);
    setContextMenu(null);
  }
  ...
  function toggleWatchedSignalContextTarget() {
    if (!contextMenu || contextMenu.kind !== 'wire') return;
    toggleWatchedSignalForWire(contextMenu.wireId);
    setContextMenu(null);
  }
