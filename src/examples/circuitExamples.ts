...
function metadataFor(example: RawCircuitExample): ExampleMetadata {
  ...
  if (example.id === 'half-adder') {
    return { ...common, moduleId: 'combinational', familyIds: ['adders', 'truth-table'], trackIds: ['arithmetic', 'architecture'], difficulty: 2, level: 'composition', prerequisites: ['and-basic', 'xor'], concepts: ['soma de bits', 'SUM', 'CARRY'], goal: 'Somar dois bits e separar o resultado em SUM e CARRY.', steps: ['Teste 0+0, 0+1, 1+0 e 1+1.', 'Observe que SUM vem do XOR.', 'Observe que CARRY vem do AND.', 'Compare cada linha com uma soma binária de um bit.'], ideas: ['SUM indica o bit de resultado.', 'CARRY indica transporte para a próxima coluna.', 'Meio somador não possui carry de entrada.'], next: ['full-adder'], observe: ['SUM liga quando A e B são diferentes.', 'CARRY só liga quando A=1 e B=1.', 'A tabela verdade tem duas saídas.'], experiments: ['Some 1+1 e veja que o resultado é 10 em binário: SUM=0 e CARRY=1.', 'Explique por que XOR resolve o SUM.', 'Explique por que AND resolve o CARRY.'], challenge: 'Substitua o XOR e o AND por um bloco Meio Somador nativo e compare as saídas.', exercises: ['Calcule manualmente SUM e CARRY para todas as quatro combinações.', 'Crie um LED chamado Resultado baixo ligado ao SUM e outro chamado Transporte ligado ao CARRY.', 'Explique por que 1+1 não cabe em um único bit.'] };
  }

  if (example.id === 'full-adder') {
    return { ...common, moduleId: 'combinational', familyIds: ['adders', 'truth-table'], trackIds: ['arithmetic', 'architecture'], difficulty: 3, level: 'composition', prerequisites: ['half-adder'], concepts: ['carry de entrada', 'carry de saída', 'soma de coluna'], goal: 'Somar A, B e Cin, como acontece em uma coluna intermediária de uma soma binária.', steps: ['Teste A=1, B=0, Cin=0.', 'Agora ligue Cin e observe a diferença.', 'Teste A=1, B=1, Cin=1.', 'Observe quando Cout liga.'], ideas: ['Cin é o transporte vindo da coluna anterior.', 'Cout é o transporte enviado para a próxima coluna.', 'Somadores maiores são feitos conectando Cout de uma coluna ao Cin da próxima.'], next: ['adder-2-bit'], observe: ['SUM pode mudar quando Cin muda.', 'Cout liga quando a soma da coluna passa de 1.', 'A tabela verdade tem três entradas e duas saídas.'], experiments: ['Compare 1+0 sem Cin e com Cin.', 'Procure as combinações em que Cout=1.', 'Explique o circuito como duas somas parciais.'], challenge: 'Monte um somador completo usando dois meio somadores e uma porta OR.', exercises: ['Preencha a tabela de 8 linhas com A, B, Cin, SUM e Cout.', 'Encontre todas as combinações que produzem resultado binário 10.', 'Explique em palavras o que Cin representa.'] };
  }

  if (example.id === 'adder-2-bit') {
    return { ...common, moduleId: 'combinational', familyIds: ['adders', 'truth-table'], trackIds: ['arithmetic', 'architecture'], difficulty: 3, level: 'composition', prerequisites: ['half-adder', 'full-adder'], concepts: ['número de 2 bits', 'propagação de carry', 'composição hierárquica'], goal: 'Somar dois números de 2 bits propagando o carry do bit 0 para o bit 1.', steps: ['Escolha A1A0=01 e B1B0=01.', 'Observe S0 e o carry que entra no bit 1.', 'Teste uma soma que gere Cout, como 11 + 01.', 'Leia o resultado como Cout S1 S0.'], ideas: ['A0 e B0 são os bits menos significativos.', 'O carry do primeiro bit vira Cin do segundo bit.', 'Esse padrão é a base dos somadores de 4, 8 ou mais bits.'], next: ['half-subtractor'], observe: ['O fio de CARRY do meio somador entra no Cin do somador completo.', 'O resultado final tem três bits: Cout S1 S0.', 'A tabela verdade tem 16 combinações.'], experiments: ['Some 01 + 01.', 'Some 10 + 01.', 'Some 11 + 01 e veja Cout aparecer.'], challenge: 'Adicione mais um somador completo para transformar este circuito em um somador de 3 bits.', exercises: ['Calcule 01 + 10 e confira S1S0.', 'Calcule 11 + 11 e confira Cout S1 S0.', 'Desenhe no papel a conexão necessária para criar um somador de 3 bits.'] };
  }

  if (example.id === 'half-subtractor') {
    return { ...common, moduleId: 'combinational', familyIds: ['adders', 'truth-table'], trackIds: ['arithmetic'], difficulty: 2, level: 'composition', prerequisites: ['not-basic', 'and-basic', 'xor'], concepts: ['subtração de bits', 'DIFF', 'BORROW'], goal: 'Subtrair B de A em um único bit e entender quando é necessário pedir emprestado.', steps: ['Teste A=0 e B=0.', 'Teste A=1 e B=0.', 'Teste A=0 e B=1 e observe BORROW.', 'Teste A=1 e B=1.'], ideas: ['DIFF usa XOR, assim como SUM no meio somador.', 'BORROW liga quando tentamos fazer 0 - 1.', 'Subtração também pode ser descrita por tabela verdade.'], next: ['full-subtractor'], observe: ['DIFF indica o bit de diferença.', 'BORROW indica empréstimo para a próxima coluna.', 'O caso A=0,B=1 é o mais importante.'], experiments: ['Compare meio somador e meio subtrator.', 'Explique por que DIFF é igual ao XOR.', 'Procure a única linha que liga BORROW.'], challenge: 'Explique por que BORROW = !A AND B.', exercises: ['Preencha a tabela de A, B, DIFF e BORROW.', 'Monte BORROW usando NOT e AND em uma aba vazia.', 'Crie um enunciado real para “precisar emprestar”.'] };
  }

  if (example.id === 'full-subtractor') {
    return { ...common, moduleId: 'combinational', familyIds: ['adders', 'truth-table'], trackIds: ['arithmetic'], difficulty: 3, level: 'composition', prerequisites: ['half-subtractor'], concepts: ['borrow de entrada', 'borrow de saída', 'subtração em colunas'], goal: 'Subtrair A - B - Bin, considerando um empréstimo vindo da coluna anterior.', steps: ['Teste Bin=0 e compare com o meio subtrator.', 'Ligue Bin e veja como a diferença muda.', 'Procure combinações que ligam Bout.', 'Compare Bin com Cin do somador completo.'], ideas: ['Bin é o empréstimo que chegou da coluna anterior.', 'Bout é o empréstimo enviado para a próxima coluna.', 'Subtratores maiores encadeiam Bout para Bin.'], next: ['comparator-1-bit'], observe: ['DIFF depende de A, B e Bin.', 'Bout liga quando a coluna atual precisa emprestar.', 'A tabela verdade tem 8 linhas.'], experiments: ['Compare A=1,B=0 com Bin=0 e Bin=1.', 'Procure todas as linhas que ligam Bout.', 'Explique o circuito como uma subtração em coluna.'], challenge: 'Monte um subtrator de 2 bits encadeando meio subtrator e subtrator completo.', exercises: ['Preencha a tabela de 8 linhas.', 'Calcule 0 - 0 - 1 e explique o empréstimo.', 'Desenhe como seria um subtrator de 2 bits.'] };
  }

  if (example.id === 'comparator-1-bit') {
    return { ...common, moduleId: 'combinational', familyIds: ['adders', 'truth-table'], trackIds: ['arithmetic', 'selection'], difficulty: 2, level: 'composition', prerequisites: ['and-basic', 'not-basic', 'xnor-basic'], concepts: ['maior que', 'igualdade', 'menor que'], goal: 'Comparar dois bits e indicar se A é maior, igual ou menor que B.', steps: ['Teste A=0,B=0.', 'Teste A=1,B=0.', 'Teste A=0,B=1.', 'Teste A=1,B=1 e observe EQ.'], ideas: ['GT liga quando A=1 e B=0.', 'LT liga quando A=0 e B=1.', 'EQ liga quando os bits são iguais, como uma XNOR.'], next: ['mux-2-1'], observe: ['Apenas uma saída deve ligar por vez.', 'EQ liga para 00 e 11.', 'GT e LT são casos opostos.'], experiments: ['Compare este circuito com o XNOR básico.', 'Explique por que GT usa A AND !B.', 'Explique por que LT usa !A AND B.'], challenge: 'Pense em como comparar números de 2 bits começando pelo bit mais significativo.', exercises: ['Crie uma tabela com A, B, GT, EQ e LT.', 'Monte apenas a saída GT em uma aba vazia.', 'Monte apenas a saída LT em uma aba vazia.'] };
  }
